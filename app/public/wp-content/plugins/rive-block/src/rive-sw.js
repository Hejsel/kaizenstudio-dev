 ...
 *
 * Scope: /wp-content/plugins/rive-block/ (can cache all plugin assets + Media Library .riv files)
 *
 * @version 2.0.0
 */
 ...
 * - .riv files: Cache Storage (HTTP Response caching) - from plugin OR uploads
...
self.addEventListener('fetch', (event) => {
	...
	// Check if this is a WASM file (from plugin build directory)
	const isWASM = url.pathname.includes('/rive-block/') && url.pathname.endsWith('.wasm');

	// Check if this is a .riv file (from plugin OR Media Library uploads)
	const isRivFile = url.pathname.endsWith('.riv');

	// Check if this is view.js (from plugin build directory)
	const isViewJS = url.pathname.includes('/rive-block/') && url.pathname.includes('/view.js');

	if (!isWASM && !isRivFile && !isViewJS) {
	...
	// .riv files and view.js: Use Cache Storage
	else {
...
/**
 * Handle .riv files and JavaScript using Cache Storage
 * Standard caching for animation files and scripts
 * NOTE: Can now cache .riv files from /wp-content/uploads/ (Media Library)!
 */
